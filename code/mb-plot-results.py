...
def main(options, logger):
    ...
    #os.mkdir(os.path.join(options.plot_dir,"cx_k_mutpb"))
    #os.mkdir(os.path.join(options.plot_dir,"cx_mutpb"))
    #os.mkdir(os.path.join(options.plot_dir,"k_mutpb"))
    #os.mkdir(os.path.join(options.plot_dir,"cx_k"))

    main_df = pd.read_csv(options.results,sep="\t", index_col=0)
    ...
    avg_df.to_csv(os.path.join(options.plot_dir,"avg_df.tsv"),sep="\t", index=False)
    max_df["mean_1"] = max_df["mean_1"].astype(int)
    max_df["t_size"] = max_df["t_size"].astype(int)
    max_df["k"] = max_df["k"].astype(int)
    max_df.to_csv(os.path.join(options.plot_dir,"max_df.tsv"),sep="\t", index=False)

    #Find the fittest individuals
    max_fit = max([row["max_val"] for i, row in max_df.iterrows()])
    with open(os.path.join(options.plot_dir,"ga_knn_fittest.txt"),"w") as f:
        f.write(str(max_fit)+"\n")
    ...
    for cx in list(set(main_df["cx"])):
        ...
        for k in list(set(main_df["k"])):
            ...
            for mut_pb in list(set(main_df["mut_pb"])):
                ...
                PLOT_cx_k_mutpb(cx_k_mutpb_df,cx,k,mut_pb,"MAXF",options.plot_dir,"max_val")
    for cx in list(set(main_df["cx"])):
        cx_df = max_df[max_df["cx"]==cx]
        for mut_pb in list(set(main_df["mut_pb"])):
            cx_mutpb_df = cx_df[cx_df["mut_pb"]==mut_pb]
            PLOT_cx_mutpb(cx_mutpb_df,cx,mut_pb,"MAXF",options.plot_dir,"max_val")
    for k in list(set(main_df["k"])):
        k_df = max_df[max_df["k"]==k]
        for mut_pb in list(set(main_df["mut_pb"])):
            k_mutpb_df = k_df[k_df["mut_pb"]==mut_pb]
            PLOT_k_mutpb(k_mutpb_df,k,mut_pb,"MAXF",options.plot_dir,"max_val")
    for cx in list(set(main_df["cx"])):
        cx_df = max_df[max_df["cx"]==cx]
        for k in list(set(main_df["k"])):
            cx_k_df = cx_df[cx_df["k"]==k]
            PLOT_cx_k(cx_k_df,cx,k,"MAXF",options.plot_dir,"max_val")
    ...
    for cx in list(set(main_df["cx"])):
        ...
        for k in list(set(main_df["k"])):
            ...
            for mut_pb in list(set(main_df["mut_pb"])):
                cx_k_mutpb_df = cx_k_df[cx_k_df["mut_pb"]==mut_pb]
                PLOT_cx_k_mutpb(cx_k_mutpb_df,cx,k,mut_pb,"AVGF",options.plot_dir,"avg_val")
    for cx in list(set(main_df["cx"])):
        cx_df = avg_df[avg_df["cx"]==cx]
        for mut_pb in list(set(main_df["mut_pb"])):
            cx_mutpb_df = cx_df[cx_df["mut_pb"]==mut_pb]
            PLOT_cx_mutpb(cx_mutpb_df,cx,mut_pb,"AVGF",options.plot_dir,"avg_val")
    for k in list(set(main_df["k"])):
        k_df = avg_df[avg_df["k"]==k]
        for mut_pb in list(set(main_df["mut_pb"])):
            k_mutpb_df = k_df[k_df["mut_pb"]==mut_pb]
            PLOT_k_mutpb(k_mutpb_df,k,mut_pb,"AVGF",options.plot_dir,"avg_val")
    for cx in list(set(main_df["cx"])):
        cx_df = avg_df[avg_df["cx"]==cx]
        for k in list(set(main_df["k"])):
            cx_k_df = cx_df[cx_df["k"]==k]
            PLOT_cx_k(cx_k_df,cx,k,"AVGF",options.plot_dir,"avg_val")
